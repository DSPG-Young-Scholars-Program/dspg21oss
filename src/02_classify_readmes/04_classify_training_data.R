...
# repo_stats data 
setwd(path_for_data)
repo_stats <- read_csv("repo_stats_0713.csv")

readme_subset_data <- readme_raw_data %>% 
  left_join(repos_table, by = "slug") %>% 
...
classified_with_readmes <- readme_subset_data %>% 
  ...
  # parse the variables down to what we have finished 
  select(slug, description, readme_text, topics, language, commits, forks, stars, watchers,
         prog_python, prog_scala, prog_rlang, prog_julia, prog_stat_all,
         prog_javascript, prog_java, prog_typescript, prog_kotlin, prog_lua, prog_web_all,
         sys_windows, sys_linux, sys_mac, sys_android, sys_virtual, starts_with("app_")) %>% 
...
python_data <- all_classified_data %>% 
  ...
  #rowwise(slug, description, readme_text) %>% 
  #summarise(python = sum(c(prog_python_desc, prog_python_lang, prog_python_read, prog_python_tops)))
